...
<body>
    <h1>更改商品</h1>
    <form action='<?=ADMIN_PATH?>/goods/update/<?=$data->id?>?action=update' method="post">
        <label>商品名</label><input type="text" name="name" value="<?=$data->name?>" />
        <label>商品图片</label><input type="text" name="img" value="<?=$data->img?>" /><input type="file" id="inputfile" name="" class="photo">
        <label>商品类型</label>
        <select name="type">
